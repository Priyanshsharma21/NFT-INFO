  ...
  return (
    <>
      ...
      ${reverse?styles.boxReverseClass:styles.boxClass} w-11/12 sm:w-full minmd:w-3/4`}>
        ...
        `}>{title}</h1>
        <p
         className={`${styles.descriptionText} ${reverse?styles.blackText:styles.whiteText}`}>{description}</p>
        {showBtn && (
          <Button assetUrl={assets.expo} link="https://expo.dev/@priyansh_sharma/nft-info?serviceType=classic&distribution=expo-go"/>
        )}
        </div>
        <div
